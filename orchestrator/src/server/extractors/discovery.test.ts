...
describe("extractor discovery", () => {
  it("finds manifest.ts and src/manifest.ts files", async () => {
    ...
    await mkdir(join(root, "workingnomads", "src"), { recursive: true });
    await mkdir(join(root, "jobspy"), { recursive: true });

    await writeFile(
      join(root, "workingnomads", "src", "manifest.ts"),
      "export const manifest = { id: 'workingnomads', displayName: 'Working Nomads', providesSources: ['workingnomads'], async run() { return { success: true, jobs: [] }; } };",
      "utf8",
    );
    await writeFile(
